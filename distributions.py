...
class LearnableDiagonalGaussian(Gaussian):
    ...
    def __init__(self, batch_size, z_size, name, mean_init=0.0,
        ...
        if var_is_trainable:
            ...
            z_var_1xn = tf.nn.sigmoid(
                z_logit_var_1xn)*(var_max-var_min) + var_min
    ...
    def __init__(self, x_bxu, z_size, name, var_min=0.0):
        ...
        Args:
          ...
          var_min (optional): Minimal variance allowed.  This is an additional
            ...
            layer.
        """
        self.size_bxn = tf.stack([tf.shape(x_bxu)[0], z_size])
...
#         self.sample_bxn = mean_bxn + tf.exp(0.5 * logvar_bxn) * noise_bxn
    ...
    @property
    def sample(self):
        noise_bxn = noise_bxn = tf.random_normal(self.size_bxn)
        noise_bxn.set_shape([None, self.z_size])
        sample_bxn = self.mean_bxn + tf.exp(0.5 * self.logvar_bxn) * noise_bxn
        return sample_bxn
    ...
    def __init__(self, zs, prior_zs):
        ...
        for z, prior_z in zip(zs, prior_zs):
            ...
            kl_b += 0.5 * tf.reduce_sum(
                ...
                + tf.square((z.mean - prior_z.mean) /
                            tf.exp(0.5 * prior_z.logvar))
                - 1.0, [1])
