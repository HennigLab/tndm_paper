...
# ==============================================================================
"""
LFADS - Preferential Latent Factor Analysis via Dynamical Systems.
...
With LFADS, there are two types of observed data: the neural 
...
The nested dictionary is the DATA DICTIONARY, which has the following keys:
  ...
  The data dictionary also has a few more keys:
    ...
    'train_behaviour' and 'valid_behaviour', if there are known beahviours
     associated with the observed neural activity, these take on dimensions:
      ExTxB, E - # examples, T - # time steps, B - # dimensions in behaviour.
    ...
    def __init__(self, hps, kind="train", datasets=None):
        ...
        for t in range(num_steps):
            ...
            with tf.variable_scope("glm", reuse=True if t > 0 else None):
                if hps.output_dist == 'poisson':
                    log_rates_t = tf.matmul(
                        factors[t], this_out_fac_W) + this_out_fac_b
        ...
        self.example_image = tf.placeholder(tf.float32, shape=[1, None, None, 3],
                                            name='image_tensor')
        self.example_summ = tf.summary.image("LFADS_example", self.example_image,
      ...
      l2,%.5f, klweight,%.2f, l2weight,%.2f\n" % \
        ...
        if self.hps.csv_log:
            csv_file = os.path.join(
                self.hps.lfads_save_dir, self.hps.csv_log+'.csv')
