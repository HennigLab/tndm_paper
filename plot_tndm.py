...
def all_plot(d, full_name="", exclude="", nspaces=0):
    """Recursively plot all the TNDM model parameters in the nested
    dictionary."""
    for k, v in d.iteritems():
        this_name = full_name+"/"+k
        if isinstance(v, dict):
            all_plot(v, full_name=this_name,
                     exclude=exclude, nspaces=nspaces+4)
        else:
            if exclude == "" or exclude not in this_name:
                _plot_item(v, name=k, full_name=full_name +
                           "/"+k, nspaces=nspaces+4)
...
def plot_tndm_timeseries(data_bxtxn, behaviours_bxtxn, model_vals, ext_input_bxtxi=None,
                         truth_bxtxn=None, truth_behaviour_bxtxn=None, bidx=None,
                         output_dist="poisson", behaviour_output_dist='gaussian',
    ...
    if output_dist == 'poisson':
        ...
        plot_time_series(rates, bidx, n_to_plot=n_to_plot, scale=scale,
                         title=col_title + " rates (TNDM - red, Truth - black)")
        ...
        plot_time_series(means, bidx, n_to_plot=n_to_plot, scale=scale,
                         title=col_title + " means (TNDM - red, Truth - black)")
    ...
    plot_time_series(means, bidx,
                     ...
                     title=col_title + " Spikes (TNDM - red, Spikes - black)")
    plot_time_series(data_bxtxn, bidx, n_to_plot=n_to_plot,
                     color='k', scale=1.0)
    ...
    plot_time_series(model_vals['rel_gen_states'], bidx, n_to_plot=n_to_plot,
                     color='g', scale=1.0, title=col_title + " Rel Generator State")
    ...
    plt.subplot(nrows, 2, 15+subplot_cidx)
    plt.imshow(means_nxt, aspect='auto', interpolation='nearest')
    plt.title(col_title + ' Means')


def plot_tndm(train_bxtxd, train_behaviours_bxtxdb, train_model_vals,
              train_ext_input_bxtxi=None, train_truth_bxtxd=None,
              train_truth_behaviours_bxtxdb=None, valid_bxtxd=None,
              valid_behaviours_bxtxdb=None, valid_model_vals=None,
              valid_ext_input_bxtxi=None, valid_truth_bxtxd=None,
              valid_truth_behaviours_bxtxdb=None, bidx=None, cf=1.0,
              output_dist='poisson', behaviour_output_dist='gaussian'):

    # Plotting
    f = plt.figure(figsize=(18, 20), tight_layout=True)
    plot_tndm_timeseries(train_bxtxd, train_behaviours_bxtxdb,
                         train_model_vals,
                         train_ext_input_bxtxi,
                         truth_bxtxn=train_truth_bxtxd,
                         truth_behaviour_bxtxn=train_truth_behaviours_bxtxdb,
                         conversion_factor=cf, bidx=bidx,
                         output_dist=output_dist,
                         behaviour_output_dist=behaviour_output_dist,
                         col_title='Train')
    plot_tndm_timeseries(valid_bxtxd, valid_behaviours_bxtxdb,
                         valid_model_vals, valid_ext_input_bxtxi,
