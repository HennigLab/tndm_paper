...
FACTORS_DIM = 50 # All factors are relevant to behaviour unless specified.
REL_FACTORS_DIM = FACTORS_DIM # Number of behaviour factors should be small compared to neural factors
...
BEHAVIOUR_OUTPUT_DISTRIBUTION = 'gaussian_non_temporal_causal'  # 'gaussian' or 'categorical'
...
TNDM_SAVE_DIR = "/tmp/tndm_chaotic_rnn_inputs_g1p5/"
CO_DIM = 1
...
REL_IC_PRIOR_VAR_MIN = 0.1
REL_IC_PRIOR_VAR_SCALE = 0.1
REL_IC_PRIOR_VAR_MAX = 0.1
REL_IC_POST_VAR_MIN = 0.0001      # protection from KL blowing up
EPSILON=1e-1

# hyperparamter that controls the relative importance of behaviour decoding
BEHAVIOUR_WEIGHT = 1.0
ORTHOGONAL_WEIGHT = 1.0
INDEPENDENCE_WEIGHT = 1.0
NUM_INDEPENDENCE_BATCHES = 10
...
flags.DEFINE_string("behaviour_output_dist", BEHAVIOUR_OUTPUT_DISTRIBUTION,
                    "Type of output distribution, 'poisson' or 'gaussian'")
...
flags.DEFINE_string("tndm_save_dir", TNDM_SAVE_DIR, "model save dir")
...
flags.DEFINE_integer("rel_ic_dim", REL_IC_DIM, "Dimension of r_h0")
...
flags.DEFINE_integer("rel_factors_dim", REL_FACTORS_DIM,
                     "Number of behaviourally relevant factors from generator")
...
# for the TNDM application or not.
flags.DEFINE_integer("gen_dim", GEN_DIM,
                     "Cell hidden size, generator.")
flags.DEFINE_integer("rel_gen_dim", REL_GEN_DIM,
                     "Cell hidden size, rel generator.")
...
# defaults should be fine for most cases, regardless of other parameters.
#
...
flags.DEFINE_float("rel_ic_prior_var_min", REL_IC_PRIOR_VAR_MIN,
                   "Minimum variance in posterior b_h0 codes.")
flags.DEFINE_float("rel_ic_prior_var_scale", REL_IC_PRIOR_VAR_SCALE,
                   "Variance of rel ic prior distribution")
flags.DEFINE_float("rel_ic_prior_var_max", REL_IC_PRIOR_VAR_MAX,
                   "Maximum variance of rel IC prior distribution.")
...
flags.DEFINE_float("rel_ic_post_var_min", REL_IC_POST_VAR_MIN,
                   "Minimum variance of rel IC posterior distribution.")
...
# (along with controller encoders placed into the TNDM graph.  If CO_DIM >
...
flags.DEFINE_float("epsilon", EPSILON,
                   "epsilon for adam.")
...
flags.DEFINE_float("l2_rel_gen_scale", L2_REL_GEN_SCALE,
                   "L2 regularization cost for the rel generator only.")
...
# UNDERFITTING
# If the primary task of TNDM is "filtering" of data and not
...
flags.DEFINE_float("kl_rel_ic_weight", KL_REL_IC_WEIGHT,
                   "Strength of KL weight on rel initial conditions KL penatly.")
...
# BALANCING BEHAVIOUR AND NEURAL ACTIVITY
# This hyperparameter controls the relative importance of the behaviour decoding
# and the neural activity reconstruction. By setting it to 1.0, the two will
# be equally important. This hyperparameter may need to be adjusted based on the
# magnitude of the behaviour and the neural activity (poisson vs gaussian for example.)
# Set relative importance of behaviour decoding
flags.DEFINE_float("behaviour_weight", BEHAVIOUR_WEIGHT,
                   "Relative importance behaviour vs. neural reconstruction.")
flags.DEFINE_float("orthogonal_weight", ORTHOGONAL_WEIGHT,
                   "Orthgonal weight between irrelevant and rel subspaces.")
flags.DEFINE_float("independence_weight", INDEPENDENCE_WEIGHT,
                   "Independent weight between irrelevant and rel subspaces.")
flags.DEFINE_integer("num_independence_batches", NUM_INDEPENDENCE_BATCHES,
                   "Number of batches of samples to compute independence.")
# Set random seed (by default will not set a seed)
...
def build_model(hps, kind="train", datasets=None):
    ...
    Args:
      ...
      kind: (optional) The kind of model to build.  Training vs inference require
        different graphs.
      datasets: The datasets structure (see top of tndm.py).
    ...
    if hps.seed != -1:
        print("Setting random seed to {}".format(hps.seed))
    ...
    if build_kind == "write_model_params":
        build_kind = "train"
    with tf.variable_scope("TNDM", reuse=None):
        model = TNDM(hps, kind=build_kind, datasets=datasets)

    if not os.path.exists(hps.tndm_save_dir):
        print("Save directory %s does not exist, creating it." %
              hps.tndm_save_dir)
        os.makedirs(hps.tndm_save_dir)

    cp_pb_ln = hps.checkpoint_pb_load_name
    cp_pb_ln = 'checkpoint' if cp_pb_ln == "" else cp_pb_ln
    if cp_pb_ln == 'checkpoint':
        print("Loading latest training checkpoint in: ", hps.tndm_save_dir)
        saver = model.seso_saver
    elif cp_pb_ln == 'checkpoint_lve':
        print("Loading lowest validation checkpoint in: ", hps.tndm_save_dir)
        saver = model.lve_saver
    else:
        print("Loading checkpoint: ", cp_pb_ln, ", in: ", hps.tndm_save_dir)
        saver = model.seso_saver

    ckpt = tf.train.get_checkpoint_state(hps.tndm_save_dir,
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
      ...
      initialized model!")
            ...
            print("Are you sure you sure a checkpoint in ", hps.tndm_save_dir,
                  " exists?")

        tf.global_variables_initializer().run()

    if ckpt:
        train_step_str = re.search(
            '-[0-9]+$', ckpt.model_checkpoint_path).group()
    else:
        train_step_str = '-0'

    fname = 'hyperparameters' + train_step_str + '.txt'
    hp_fname = os.path.join(hps.tndm_save_dir, fname)
...
def build_hyperparameter_dict(flags):
    ...
    d['behaviour_output_dist'] = flags.behaviour_output_dist

    d['data_dir'] = flags.data_dir
    d['tndm_save_dir'] = flags.tndm_save_dir
    ...
    d['rel_ic_dim'] = flags.rel_ic_dim
    d['factors_dim'] = flags.factors_dim
    d['rel_factors_dim'] = flags.rel_factors_dim
    ...
    d['rel_gen_dim'] = flags.rel_gen_dim
    ...
    d['rel_ic_prior_var_min'] = flags.rel_ic_prior_var_min
    d['rel_ic_prior_var_scale'] = flags.rel_ic_prior_var_scale
    d['rel_ic_prior_var_max'] = flags.rel_ic_prior_var_max
    d['rel_ic_post_var_min'] = flags.rel_ic_post_var_min
    ...
    d['l2_rel_gen_scale'] = flags.l2_rel_gen_scale
    d['l2_con_scale'] = flags.l2_con_scale

    # Underfitting
    d['kl_ic_weight'] = flags.kl_ic_weight
    d['kl_rel_ic_weight'] = flags.kl_rel_ic_weight
    ...
    # Balancing behaviour and neural activity reconstruction
    d['behaviour_weight'] = flags.behaviour_weight
    d['orthogonal_weight'] = flags.orthogonal_weight
    d['independence_weight'] = flags.independence_weight
    d['num_independence_batches'] = flags.num_independence_batches
    d['seed'] = flags.seed  # random seed
...
def write_model_parameters(hps, output_fname=None, datasets=None):
    ...
    Save all the parameters to hps.tndm_save_dir.
    ...
    fname = os.path.join(hps.tndm_save_dir, output_fname)
...
def clean_data_dict(data_dict):
    ...
    Args:
      data_dict - dictionary containing data for TNDM
    Returns:
...
def load_datasets(data_dir, data_filename_stem):
    ...
    for k, data_dict in datasets.items():
        ...
        # Load behavioural datasets
        train_behaviours_total_size = len(data_dict['train_behaviours'])
        if train_behaviours_total_size == 0:
            print("Did not load behavioural training set.")
        else:
            print("Found behavioural training set with number examples: ",
                  train_behaviours_total_size)

        valid_behaviours_total_size = len(data_dict['valid_behaviours'])
        if valid_behaviours_total_size == 0:
            print("Did not load behavioural validation set.")
        else:
            print("Found behavioural validation set with number examples: ",
                  valid_behaviours_total_size)
...
def main(_):
    ...
    hps.dataset_names = []
    hps.dataset_dims = {}
    hps.behaviour_dataset_dims = {}
    for key in datasets:
        print(datasets[key].keys())
        hps.dataset_names.append(key)
        hps.dataset_dims[key] = datasets[key]['data_dim']
        hps.behaviour_dataset_dims[key] = datasets[key]['behaviour_dataset_dims']
