...
# ==============================================================================
"""
TNDM - Targeted Neural Dynamical Modelling
TNDM is an unsupervised method to decompose time series data into
...
With TNDM, there are two types of observed data: the neural 
activity and the associated behaviour of interest. These 
observations have a noise model (Poisson or Gaussian), so a denoised 
version of the observations is also created (e.g. underlying rates 
of a Poisson distribution given the observed spike counts).
...
The nested dictionary is the DATA DICTIONARY, which has the following keys:
  ...
  The data dictionary also has a few more keys:
    'train_ext_input' and 'valid_ext_input', if there are known external inputs
      to the system being modeled, these take on dimensions:
      ExTxI, E - # examples, T - # time steps, I = # dimensions in input.
    'train_behaviours' and 'valid_behaviours', if there are known beahviours
     associated with the observed neural activity, these take on dimensions:
      ExTxB, E - # examples, T - # time steps, B - # dimensions in behaviours.
  ...
  If one runs TNDM on data where the true rates are known for some trials,
...
from plot_tndm import plot_tndm
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):  # "GRU"
            ...
            new_h = tf.clip_by_value(
                new_h, -self._clip_value, self._clip_value)
    ...
    def __init__(self, hps, kind="train", datasets=None):
        """Create an TNDM model.
           ...
           posterior_sample_and_average - sample from the posterior, this is used
             for evaluating the expected value of the outputs of TNDM, given a
        ...
        Args:
          ...
          datasets: a dictionary of named data_dictionaries, see top of TNDM.py
        """
        ...
        if hps.behaviour_output_dist == 'gaussian' or hps.behaviour_output_dist == 'gaussian_non_temporal' or hps.behaviour_output_dist == 'gaussian_non_temporal_causal' :
            # Enforce correct dtype
            assert np.issubdtype(
                datasets[hps.dataset_names[0]]['train_behaviours'].dtype, float), \
                "Data dtype must be float for gaussian output distribution"
            behaviour_data_dtype = tf.float32
        elif hps.behaviour_output_dist == 'categorical':
            assert np.issubdtype(
                datasets[hps.dataset_names[0]]['train_behaviours'].dtype, int), \
                "Data dtype must be int for categorical output dsitribution"
            behaviour_data_dtype = tf.int32
        else:
            assert False, "NIY"
        ...
        self.dataset_ph = dataset_ph = tf.placeholder(data_dtype,
                                                      [None, num_steps, None],
                                                      name="data")
        self.behaviour_dataset_ph = behaviour_dataset_ph = tf.placeholder(behaviour_data_dtype,
                                                                          [None, num_steps,
                                                                              None],
                                                                          name="behaviour_data")
        ...
        rel_factors_dim = hps.rel_factors_dim
        ...
        # relevant to behaviour factors
        self.fns_out_rel_fac_Ws = fns_out_rel_fac_Ws = [None] * ndatasets
        self.fns_out_rel_fac_bs = fns_out_rel_fac_bs = [None] * ndatasets
        ...
        for d, name in enumerate(dataset_names):
            ...
            # behaviour dimension
            behaviour_dataset_dims = hps.behaviour_dataset_dims[name]
            in_mat_cxf = None
            in_bias_1xf = None
            align_bias_1xc = None
        ...
        with tf.variable_scope("glm"):
            ...
            for d, name in enumerate(dataset_names):
                ...
                if datasets and 'alignment_matrix_cxf' in datasets[name].keys():
                    dataset = datasets[name]
                    in_mat_cxf = dataset['alignment_matrix_cxf'].astype(
                        np.float32)

                if datasets and 'alignment_bias_c' in datasets[name].keys():
                    dataset = datasets[name]
                    align_bias_c = dataset['alignment_bias_c'].astype(
                        np.float32)
                    align_bias_1xc = np.expand_dims(align_bias_c, axis=0)
                ...
                if hps.output_dist == 'poisson':
                    ...
                    mat_init_value = np.zeros(
                        [factors_dim, data_dim]).astype(np.float32)
        ...
        ###BEHAVIOUR OBSERVATION MODELS###
        with tf.variable_scope("behaviour_dec"):
            for d, name in enumerate(dataset_names):
                behaviour_dataset_dims = hps.behaviour_dataset_dims[name]
                if hps.behaviour_output_dist == 'gaussian':
                    out_rel_fac_lin_mean = \
                        init_linear(rel_factors_dim, behaviour_dataset_dims, do_bias=True,
                                    mat_init_value=None,
                                    bias_init_value=None,
                                    normalized=False,
                                    name="rel_fac_2_means_"+name,
                                    collections=['IO_transformations'])
                    out_rel_fac_W_mean, out_rel_fac_b_mean = out_rel_fac_lin_mean

                    mat_init_value = np.zeros(
                        [rel_factors_dim, behaviour_dataset_dims]).astype(np.float32)
                    bias_init_value = np.ones(
                        [1, behaviour_dataset_dims]).astype(np.float32)
                    out_rel_fac_lin_logvar = \
                        init_linear(rel_factors_dim, behaviour_dataset_dims, do_bias=True,
                                    mat_init_value=mat_init_value,
                                    bias_init_value=bias_init_value,
                                    normalized=False,
                                    name="rel_fac_2_logvars_"+name,
                                    collections=['IO_transformations'])
                    out_rel_fac_W_mean, out_rel_fac_b_mean = out_rel_fac_lin_mean
                    out_rel_fac_W_logvar, out_rel_fac_b_logvar = out_rel_fac_lin_logvar
                    out_rel_fac_W = tf.concat(
                        axis=1, values=[out_rel_fac_W_mean, out_rel_fac_W_logvar])
                    out_rel_fac_b = tf.concat(
                        axis=1, values=[out_rel_fac_b_mean, out_rel_fac_b_logvar])
                    
                    
                elif hps.behaviour_output_dist == 'gaussian_non_temporal' or hps.behaviour_output_dist == 'gaussian_non_temporal_causal':
                    #all to all linear
                    out_rel_fac_lin_mean = \
                        init_linear(rel_factors_dim*hps.num_steps, behaviour_dataset_dims*hps.num_steps, do_bias=True,
                                    mat_init_value=None,
                                    bias_init_value=None,
                                    normalized=False,
                                    name="rel_fac_2_means_"+name,
                    ...
                    out_rel_fac_W = out_rel_fac_W_mean                    
                    out_rel_fac_b = out_rel_fac_b_mean

                elif hps.behaviour_output_dist == 'categorical':
                    assert False, "categorical NIY"
                else:
                    assert False, "NIY"

                # behaviour layers
                fns_out_rel_fac_Ws[d] = makelambda(out_rel_fac_W)
                fns_out_rel_fac_bs[d] = makelambda(out_rel_fac_b)
        ...
        # behaviour layers
        pf_pairs_out_rel_fac_Ws = list(zip(preds, fns_out_rel_fac_Ws))
        pf_pairs_out_rel_fac_bs = list(zip(preds, fns_out_rel_fac_bs))
        this_out_rel_fac_W = tf.case(pf_pairs_out_rel_fac_Ws, exclusive=True)
        this_out_rel_fac_b = tf.case(pf_pairs_out_rel_fac_bs, exclusive=True)
        ...
        def encode_data(dataset_bxtxd, enc_cell, name, forward_or_reverse,
                        num_steps_to_encode, used_in_factors_dim, align=False, 
                        this_in_fac_W=None, this_in_fac_b=None):
            """Encode data for TNDM
            Args:
            ...
            for i, t in enumerate(time_fwd_or_rev):
                with tf.variable_scope(name+"_enc"+dstr, reuse=True if i > 0 else None):
                    ...
                    if align:
                        in_fac_t_bxf = tf.matmul(
                            dataset_t_bxd, this_in_fac_W) + this_in_fac_b
                    else:
                        in_fac_t_bxf = dataset_t_bxd
        ...
        if ic_dim > 0:
            ...
            ic_enc_fwd = encode_data(dataset_do_bxtxd, enc_ic_cell,
                                     ...
                                     used_in_factors_dim=used_in_factors_dim,
                                     align=True, this_in_fac_W=this_in_fac_W, 
                                     this_in_fac_b=this_in_fac_b)
        ...
        with tf.variable_scope("z", reuse=False):
            ...
            if ic_dim > 0 and rel_ic_dim > 0:
                ...
                self.prior_rel_zs_g0 = \
                    LearnableDiagonalGaussian(batch_size, rel_ic_dim, name="prior_rel_g0",
                                              mean_init=0.0,
                                              var_min=hps.rel_ic_prior_var_min,
                                              var_init=hps.rel_ic_prior_var_scale,
                                              var_max=hps.rel_ic_prior_var_max)
                ic_enc = tf.concat(
                    axis=1, values=[ic_enc_fwd[-1], ic_enc_rev[0]])
                ...
                self.posterior_rel_zs_g0 = \
                    DiagonalGaussianFromInput(ic_enc, rel_ic_dim, "ic_enc_2_post_rel_g0",
                                              var_min=hps.rel_ic_post_var_min)
                
                if kind in ["train", "posterior_sample_and_average",
                            "posterior_push_mean"]:
                    zs_g0 = self.posterior_zs_g0
                    rel_zs_g0 = self.posterior_rel_zs_g0
                else:
                    zs_g0 = self.prior_zs_g0
                    rel_zs_g0 = self.prior_rel_zs_g0
                if kind in ["train", "posterior_sample_and_average", "prior_sample"]:
                    self.g0s_val = zs_g0.sample
                    self.rel_g0s_val = rel_zs_g0.sample
                else:
                    self.g0s_val = zs_g0.mean
                    self.rel_g0s_val = rel_zs_g0.mean
            ...
            if co_dim > 0:
                ...
                autocorrelation_taus = [
                    hps.prior_ar_atau for x in range(hps.co_dim)]
                noise_variances = [
                    hps.prior_ar_nvar for x in range(hps.co_dim)]
        ...
        self.rel_gen_inputs = rel_gen_inputs = [None] * num_steps
        if co_dim > 0:
            ...
            with tf.variable_scope("con", reuse=False):
                ...
                self.con_ics.set_shape(
                    [None, con_cell.state_size])  # tile loses shape
                con_states[-1] = self.con_ics
        
        #gen cell for neural data
        ...
        #gen cell for rel data
        rel_gen_cell = gen_cell_class(hps.rel_gen_dim,
                                      input_weight_scale=hps.gen_cell_input_weight_scale,
                                      rec_weight_scale=hps.gen_cell_rec_weight_scale,
                                      clip_value=hps.cell_clip_value,
                                      recurrent_collections=['l2_rel_gen_reg'])
        ...
        with tf.variable_scope("gen", reuse=False):
            ...
            irel_factors = linear(gen_outs[-1], factors_dim - rel_factors_dim, do_bias=False,
                                  normalized=True, name="gen_2_irel_fac")
                
        with tf.variable_scope("rel_gen", reuse=False):
            if rel_ic_dim == 0:
                self.rel_gen_ics = tf.tile(
                    tf.Variable(tf.zeros([1, rel_gen_cell.state_size]), name="rel_g0"),
                    tf.stack([batch_size, 1]))
            else:
                self.rel_gen_ics = linear(self.rel_g0s_val, rel_gen_cell.state_size,
                                          identity_if_possible=True,
                                          name="rel_g0_2_rel_gen_ic")

            self.rel_gen_states = rel_gen_states = [None] * num_steps
            self.rel_gen_outs = rel_gen_outs = [None] * num_steps
            rel_gen_states[-1] = self.rel_gen_ics
            rel_gen_outs[-1] = rel_gen_cell.output_from_state(rel_gen_states[-1])
            rel_factors = linear(rel_gen_outs[-1], rel_factors_dim, do_bias=False,
                                 normalized=True, name="rel_gen_2_rel_fac")
            
        #pair with relevant latent from other generator
        factors[-1] = tf.concat([rel_factors, irel_factors], 1)
        ...
        with tf.variable_scope("glm", reuse=False):
            if hps.output_dist == 'poisson':
                log_rates_t0 = tf.matmul(
                    factors[-1], this_out_fac_W) + this_out_fac_b
                log_rates_t0.set_shape([None, None])
                rates[-1] = tf.exp(log_rates_t0)  # rate
                rates[-1].set_shape([None,
                                    hps.dataset_dims[hps.dataset_names[0]]])
            elif hps.output_dist == 'gaussian':
                mean_n_logvars = tf.matmul(
                    factors[-1], this_out_fac_W) + this_out_fac_b
        ...
        # batch x (D+D+D).# with tf.variable_scope("behaviour_dec", reuse=False):
        self.output_dist_params = dist_params = [None] * num_steps
        self.behaviour_output_dist_params = behaviour_dist_params = [
            None] * num_steps
        self.log_p_xgz_b = log_p_xgz_b = 0.0  # log P(x|z), x=neural activity
        self.log_p_ygz_b = log_p_ygz_b = 0.0  # log P(y|z), y=behaviour
        self.behaviour_weight = behaviour_weight = tf.constant(
            hps.behaviour_weight)
        self.orthogonal_weight = orthogonal_weight = tf.constant(
            hps.orthogonal_weight)
        self.independence_weight = independence_weight = tf.constant(hps.independence_weight)
        self.num_independence_batches = num_independence_batches = hps.num_independence_batches
        behaviour_data_bxdb = behaviour_dataset_ph
        for t in range(num_steps):
            ...
            if co_dim > 0:
                ...
                with tf.variable_scope("con", reuse=True if t > 0 else None):
                    con_outs[t], con_states[t] = con_cell(
                        con_in_t, con_states[t-1])
                    posterior_zs_co[t] = \
            ...
            if ext_input_dim > 0 and hps.inject_ext_input_to_gen:
                ext_input_t_bxi = ext_input_do_bxtxi[:, t, :]
                if co_dim > 0:
                    gen_inputs[t] = tf.concat(
                        axis=1, values=[u_t[t], ext_input_t_bxi])
                else:
            ...
            behaviour_data_t_bxdb = behaviour_dataset_ph[:, t, :]
            
            #irel generator  
            with tf.variable_scope("gen", reuse=True if t > 0 else None):
                gen_outs[t], gen_states[t] = gen_cell(
                    gen_inputs[t], gen_states[t-1])
                gen_outs[t] = tf.nn.dropout(gen_outs[t], keep_prob)
            with tf.variable_scope("gen", reuse=True):  # ic defined it above
                irel_factors = linear(gen_outs[t], factors_dim - rel_factors_dim, do_bias=False,
                                 normalized=True, name="gen_2_irel_fac")
                
            #rel generator  
            with tf.variable_scope("rel_gen", reuse=True if t > 0 else None):
                rel_gen_outs[t], rel_gen_states[t] = rel_gen_cell(
                    rel_gen_inputs[t], rel_gen_states[t-1])
                rel_gen_outs[t] = tf.nn.dropout(rel_gen_outs[t], keep_prob)
            with tf.variable_scope("rel_gen", reuse=True):  # ic defined it above
                rel_factors = linear(rel_gen_outs[t], rel_factors_dim, do_bias=False,
                                     normalized=True, name="rel_gen_2_rel_fac") 
                
            factors[t] = tf.concat([rel_factors, irel_factors], 1)
                                  
            with tf.variable_scope("glm", reuse=True if t > 0 else None):
                if hps.output_dist == 'poisson':
                    log_rates_t = tf.matmul(factors[t], this_out_fac_W) + this_out_fac_b
                    # log_rates_t = logrates_rel (valid rates) + logrates_irel (valid rates)
                    log_rates_t.set_shape([None, None])
                    rates[t] = dist_params[t] = tf.exp(tf.clip_by_value(
                        log_rates_t, -hps._clip_value, hps._clip_value))  # rates feed back
                    rates[t].set_shape(
                        [None, hps.dataset_dims[hps.dataset_names[0]]])
                    ...
                    assert False, "NIY"
                log_p_xgz_b += tf.reduce_sum(loglikelihood_t, [1]) # neural likelihood

            if hps.behaviour_output_dist == 'gaussian':
                with tf.variable_scope("behaviour_dec", reuse=True if t > 0 else None):
                    # Only uses relevant factors to reconstruct behaviour
                    mean_n_logvars = tf.matmul(
                        factors[t][:, :rel_factors_dim], this_out_rel_fac_W) + this_out_rel_fac_b
                    mean_n_logvars.set_shape([None, None])
                    behaviour_means_t_bxdb, behaviour_logvars_t_bxdb = tf.split(axis=1, num_or_size_splits=2,
                                                                                value=mean_n_logvars)
                    behaviour_dist_params[t] = tf.concat(
                        axis=1, values=[behaviour_means_t_bxdb, tf.exp(tf.clip_by_value(behaviour_logvars_t_bxdb, -hps._clip_value, hps._clip_value))])
                    #turn off variances
                    loglikelihood_t = tf.square(behaviour_data_t_bxdb - behaviour_means_t_bxdb)
                    # behaviour likelihood
                    log_p_ygz_b += - tf.reduce_sum(loglikelihood_t, [1])

        # Support multiple behavioural output distributions in theory,
        # for example Gaussian and also Categorical.
#         print(hps.behaviour_output_dist)
        if hps.behaviour_output_dist == 'gaussian':
            pass
        elif hps.behaviour_output_dist == 'gaussian_non_temporal':
            with tf.variable_scope("behaviour_dec", reuse=tf.AUTO_REUSE):
                # Uses full relevant factors to reconstruct behaviour at all time points
                rel_factors_tensor = tf.transpose(tf.convert_to_tensor(factors)[:,:,:rel_factors_dim], perm=[1,2,0])
                rel_factors_tensor_reshaped = tf.reshape(rel_factors_tensor, shape=(-1, hps.num_steps*rel_factors_dim))
                behaviour_means_bxdb = tf.matmul(rel_factors_tensor_reshaped, this_out_rel_fac_W) + this_out_rel_fac_b
                behaviour_means_bxdb_reshaped = tf.stack(tf.split(axis=1, num_or_size_splits=behaviour_dataset_dims, value=behaviour_means_bxdb), axis=2)
#                 print(behaviour_means_bxdb_reshaped)
                for t in range(hps.num_steps):
                    behaviour_dist_params[t] = behaviour_means_bxdb_reshaped[:,t]
                loglikelihood_t = tf.square(behaviour_data_bxdb - behaviour_means_bxdb_reshaped)
                log_p_ygz_b = - tf.reduce_mean(tf.reduce_sum(loglikelihood_t, [1]), 1)   
                
        elif hps.behaviour_output_dist == 'gaussian_non_temporal_causal':
            with tf.variable_scope("behaviour_dec", reuse=tf.AUTO_REUSE):
                # Uses full relevant factors causually to reconstruct behaviour at all time points factors -- length T - [B x num_factor_dims]
                rel_factors_tensor = tf.transpose(tf.convert_to_tensor(factors)[:,:,:rel_factors_dim], perm=[1,2,0]) # TxBxF --> B x F_r x T
                rel_factors_tensor_reshaped = tf.reshape(rel_factors_tensor, shape=(-1, hps.num_steps*rel_factors_dim)) # B x F_r * T
                rel_fac_2_means_W = this_out_rel_fac_W
                causal_rel_fac_2_means_W_list = [[] for _ in range(rel_factors_dim)]
                for i in range(rel_factors_dim):
                    curr_rel_fac_2_means_W = []
                    for j in range(behaviour_dataset_dims):
                        curr_rel_fac_2_means_W.append(tf.linalg.band_part(rel_fac_2_means_W[i*hps.num_steps:(i+1)*hps.num_steps,j*hps.num_steps:(j+1)*hps.num_steps], 0, -1))
                    causal_rel_fac_2_means_W_list[i] = tf.concat(curr_rel_fac_2_means_W, axis=1)
                causal_rel_fac_2_means_W = tf.concat(causal_rel_fac_2_means_W_list, axis=0)
                behaviour_means_bxdb = tf.matmul(rel_factors_tensor_reshaped, causal_rel_fac_2_means_W) + this_out_rel_fac_b
                behaviour_means_bxdb_reshaped = tf.stack(tf.split(axis=1, num_or_size_splits=behaviour_dataset_dims, value=behaviour_means_bxdb), axis=2)
                for t in range(hps.num_steps):
                    behaviour_dist_params[t] = behaviour_means_bxdb_reshaped[:,t]
        ...
        if hps.co_mean_corr_scale > 0.0:
            ...
            # div by batch and by n*(n-1)/2 pairs
        ...
        kl_cost_rel_g0_b = tf.zeros_like(batch_size, dtype=tf.float32)
        kl_cost_co_b = tf.zeros_like(batch_size, dtype=tf.float32)
        self.kl_cost = tf.constant(0.0)  # VAE KL cost
        self.recon_cost = tf.constant(0.0)  # reconstruction cost
        self.behaviour_recon_cost = tf.constant(0.0)  # Behaviour reconstruction cost
        ...
        if kind in ["train", "posterior_sample_and_average", "posterior_push_mean"]:
            ...
            #rel ic kl cost
            if rel_ic_dim > 0:
                rel_g0_priors = [self.prior_rel_zs_g0]
                rel_g0_posts = [self.posterior_rel_zs_g0]
                kl_cost_rel_g0_b = KLCost_GaussianGaussian(rel_g0_posts, rel_g0_priors).kl_cost_b
                kl_cost_rel_g0_b = hps.kl_rel_ic_weight * kl_cost_rel_g0_b
            
            #compute covariance between rel and irel with batches of samples from posteriors
            g0s_cov_samples = []
            rel_g0s_cov_samples = []
            for i in range(num_independence_batches):
                g0s_cov_samples.append(zs_g0.sample)
                rel_g0s_cov_samples.append(rel_zs_g0.sample)      
            g0s_cov_samples_tensor = tf.reshape(tf.stack(g0s_cov_samples), shape=(num_independence_batches*self.g0s_val.shape[0], self.g0s_val.shape[1]))
            rel_g0s_cov_samples_tensor = tf.reshape(tf.stack(rel_g0s_cov_samples), shape=(num_independence_batches*self.rel_g0s_val.shape[0], self.rel_g0s_val.shape[1]))
            cov_matrix_rel_irel = tfp.stats.covariance(g0s_cov_samples_tensor, rel_g0s_cov_samples_tensor, sample_axis=0, event_axis=-1)
            squared_cov_matrix_rel_irel = cov_matrix_rel_irel**2
            self.independence_cost = tf.reduce_mean(squared_cov_matrix_rel_irel)
            ...
            self.behaviour_recon_cost = - tf.reduce_mean(log_p_ygz_b) # Behaviour reconstruction cost
            self.kl_cost = tf.reduce_mean(kl_cost_g0_b + kl_cost_rel_g0_b + kl_cost_co_b)

            lb_on_ll_b = log_p_xgz_b + log_p_ygz_b - kl_cost_g0_b - kl_cost_rel_g0_b - kl_cost_co_b
        ...
        if self.hps.l2_gen_scale > 0.0 or self.hps.l2_rel_gen_scale > 0.0 or self.hps.l2_con_scale > 0.0:
            l2_costs = []
            l2_numels = []
            l2_reg_var_lists = [tf.get_collection('l2_gen_reg'),
                                tf.get_collection('l2_rel_gen_reg'),
                                tf.get_collection('l2_con_reg')]
            l2_reg_scales = [self.hps.l2_gen_scale, self.hps.l2_rel_gen_scale, self.hps.l2_con_scale]
            for l2_reg_vars, l2_scale in zip(l2_reg_var_lists, l2_reg_scales):
                for v in l2_reg_vars:
                    numel = tf.reduce_prod(
                        tf.concat(axis=0, values=tf.shape(v)))
        ...
        # orthogonal penalty on the neural and behavioural subspaces.
        self.orthogonal_cost = tf.constant(0.0)
#         trainable_var_key = tf.GraphKeys.TRAINABLE_VARIABLES
#         rel_factors_weights = tf.get_collection(key=trainable_var_key, scope="TNDM/behaviour_gen/behaviour_gen_2_rel_fac")[0] #behaviour_ic_dim x rel_fac (64x2) -- A
#         behaviour_factors_weights = tf.get_collection(key=trainable_var_key, scope="TNDM/behaviour_gen/behaviour_gen_2_behaviour_fac")[0] #behaviour_ic_dim x b_fac (64x1) -- B
#         rel_transpose_behaviour = tf.matmul(tf.transpose(rel_factors_weights), behaviour_factors_weights) #A^TC = 0 (1,1)
#         self.orthogonal_cost = tf.square(tf.norm(rel_transpose_behaviour))# + tf.square(tf.norm(rel_transpose_behaviour))
        ...
        # Cost includes behaviour reconstruction with hyperparam to adjust scale
        self.cost = self.recon_cost + behaviour_weight*self.behaviour_recon_cost + \
            self.timed_kl_cost + self.timed_l2_cost + self.weight_corr_cost + \
            orthogonal_weight*self.orthogonal_cost + independence_weight*self.independence_cost
        ...
        self.example_image = tf.placeholder(tf.float32, shape=[1, None, None, 3],
                                            name='image_tensor')
        self.example_summ = tf.summary.image("TNDM_example", self.example_image,
                                             collections=["example_summaries"])

        # general training summaries
        self.lr_summ = tf.summary.scalar("Learning_rate", self.learning_rate)
        self.kl_weight_summ = tf.summary.scalar("KL_weight", self.kl_weight)
        self.l2_weight_summ = tf.summary.scalar("L2_weight", self.l2_weight)
        self.orthogonal_weight_summ = tf.summary.scalar("Orthogonal_weight", self.orthogonal_weight)
        self.independence_weight_summ = tf.summary.scalar("Independence_weight", self.independence_weight)
        self.behaviour_weight_summ = tf.summary.scalar(
            "Behaviour_weight", behaviour_weight)
        self.corr_cost_summ = tf.summary.scalar(
            "Corr_cost", self.weight_corr_cost)
        self.grad_global_norm_summ = tf.summary.scalar("Gradient_global_norm",
        ...
        if hps.co_dim > 0:
            ...
            for c in range(hps.co_dim):
                self.atau_summ[c] = \
                    tf.summary.scalar("AR_Autocorrelation_taus " + str(c),
                                      tf.exp(self.prior_zs_ar_con.logataus_1xu[0, c]))
                self.pvar_summ[c] = \
                    tf.summary.scalar("AR_Variances " + str(c),
        ...
        self.kl_t_cost_summ = tf.summary.scalar("KL_cost_train", kl_cost_ph,
                                                collections=["train_summaries"])
        self.kl_v_cost_summ = tf.summary.scalar("KL_cost_valid", kl_cost_ph,
                                                collections=["valid_summaries"])
        l2_cost_ph = tf.placeholder(tf.float32, shape=[], name='l2_cost_ph')
        self.l2_cost_summ = tf.summary.scalar("L2_cost", l2_cost_ph,
                                              collections=["train_summaries"])
        orthogonal_cost_ph = tf.placeholder(tf.float32, shape=[], name='orthogonal_cost_ph')
        self.orthogonal_cost_summ = tf.summary.scalar("Orthogonal_cost", orthogonal_cost_ph,
                                                      collections=["train_summaries"])
        independence_cost_ph = tf.placeholder(tf.float32, shape=[], name='independence_cost_ph')
        self.independence_cost_ph = tf.summary.scalar("Independence_cost", independence_cost_ph,
                                                      collections=["train_summaries"])
        recon_cost_ph = tf.placeholder(
            tf.float32, shape=[], name='recon_cost_ph')
        self.recon_t_cost_summ = tf.summary.scalar("Reconstruction_cost_train",
                                                   recon_cost_ph,
                                                   collections=["train_summaries"])
        self.recon_v_cost_summ = tf.summary.scalar("Reconstruction_cost_valid",
                                                   recon_cost_ph,
                                                   collections=["valid_summaries"])

        # behaviour cost summaries
        behaviour_recon_cost_ph = tf.placeholder(
            tf.float32, shape=[], name='behaviour_recon_cost_ph')
        self.behaviour_recon_t_cost_summ = tf.summary.scalar("Behaviour_reconstruction_cost_train",
                                                             behaviour_recon_cost_ph,
                                                             collections=["train_summaries"])
        self.behaviour_recon_v_cost_summ = tf.summary.scalar("Behaviour_reconstruction_cost_valid",
                                                             behaviour_recon_cost_ph,
                                                             collections=["valid_summaries"])
        

        total_cost_ph = tf.placeholder(
            tf.float32, shape=[], name='total_cost_ph')
        self.cost_t_summ = tf.summary.scalar("Total_cost_train", total_cost_ph,
                                             collections=["train_summaries"])
        self.cost_v_summ = tf.summary.scalar("Total_cost_valid", total_cost_ph,
                                             collections=["valid_summaries"])

        self.kl_cost_ph = kl_cost_ph
        self.l2_cost_ph = l2_cost_ph
        self.orthogonal_cost_ph = orthogonal_cost_ph
        self.independence_cost_ph = independence_cost_ph
        self.recon_cost_ph = recon_cost_ph
        # Behaviour reconstruction cost placeholder
        self.behaviour_recon_cost_ph = behaviour_recon_cost_ph
        ...
        self.logfile = os.path.join(hps.tndm_save_dir, "tndm_log")
        self.writer = tf.summary.FileWriter(self.logfile)

    def build_feed_dict(self, train_name, data_bxtxd, behaviour_data_bxtxdb,
                        ext_input_bxtxi=None, keep_prob=None):
        ...
        Args:
          ...
          behaviour_data_bxtxdb: The behaviour data tensor
        ...
        feed_dict[self.behaviour_dataset_ph] = behaviour_data_bxtxdb
    ...
    @staticmethod
    def get_batch(data_extxd, behaviour_data_extxdb=None, ext_input_extxi=None,
                  batch_size=None, example_idxs=None):
        ...
        Args:
          ...
          behaviour_data_extxdb: The behaviour data to model, numpy tensors with shape:
            # examples x # time steps x # dimensions
        ...
        Returns:
          A tuple with three parts:
            ...
            2. Batched behaviour data numpy tensor with shape:
            batch_size x # time steps x # behaviour dimensions
            3. Batched external input numpy tensor with shape:
        ...
        behaviour_data_bxtxdb = None
        if behaviour_data_extxdb is not None:
            behaviour_data_bxtxdb = behaviour_data_extxdb[example_idxs, :, :]

        ext_input_bxtxi = None
        if ext_input_extxi is not None:
            ext_input_bxtxi = ext_input_extxi[example_idxs, :, :]

        return data_extxd[example_idxs, :, :], behaviour_data_bxtxdb, ext_input_bxtxi
    ...
    def shuffle_and_flatten_datasets(self, datasets, kind='train'):
        """Since TNDM supports multiple datasets in the same dynamical model,
        ...
        Args:
          datasets: A dict of data dicts.  The dataset dict is simply a
            name(string)-> data dictionary mapping (See top of tndm.py).
          kind: 'train' or 'valid'
        Returns:
        ...
        for name, data_dict in datasets.items():
            ...
            random_example_idxs, _ = \
                self.randomize_example_idxs_mod_batch_size(
                    nexamples, batch_size)
    ...
    def train_epoch(self, datasets, batch_size=None, do_save_ckpt=True):
        ...
        """
        ops_to_eval = [self.cost, self.recon_cost, self.behaviour_recon_cost,
                       self.kl_cost, self.behaviour_weight, self.kl_weight,
                       self.l2_cost, self.l2_weight, self.train_op, 
                       self.orthogonal_cost, self.orthogonal_weight,
                       self.independence_cost, self.independence_weight]
        collected_op_values = self.run_epoch(
            datasets, ops_to_eval, kind="train")

        total_cost = total_recon_cost = total_behaviour_recon_cost = total_kl_cost = 0.0
        ...
        for op_values in collected_op_values:
            ...
            total_behaviour_recon_cost += op_values[2]
            total_kl_cost += op_values[3]

        behaviour_weight = collected_op_values[-1][4]
        kl_weight = collected_op_values[-1][5]
        l2_cost = collected_op_values[-1][6]
        l2_weight = collected_op_values[-1][7]
        orthogonal_cost = collected_op_values[-1][9]
        orthogonal_weight = collected_op_values[-1][10]
        independence_cost = collected_op_values[-1][11]
        independence_weight = collected_op_values[-1][12]

        epoch_total_cost = total_cost / epoch_size
        epoch_recon_cost = total_recon_cost / epoch_size
        epoch_behaviour_recon_cost = total_behaviour_recon_cost / epoch_size
        epoch_kl_cost = total_kl_cost / epoch_size

        if do_save_ckpt:
            session = tf.get_default_session()
            checkpoint_path = os.path.join(self.hps.tndm_save_dir,
                                           self.hps.checkpoint_name + '.ckpt')
            self.seso_saver.save(session, checkpoint_path,
                                 global_step=self.train_step)

        return epoch_total_cost, epoch_recon_cost, epoch_behaviour_recon_cost, \
            epoch_kl_cost, behaviour_weight, kl_weight, l2_cost, l2_weight, \
            orthogonal_cost, orthogonal_weight, independence_cost, independence_weight
    ...
    def run_epoch(self, datasets, ops_to_eval, kind="train", batch_size=None,
        ...
        kind_data = kind + '_data'
        kind_behaviours = kind + '_behaviours'
        ...
        for name, example_idxs in all_name_example_idx_pairs:
            ...
            behaviour_data_extxdb = data_dict[kind_behaviours]
            ...
            data_bxtxd, behaviour_data_bxtxdb, ext_input_bxtxi = self.get_batch(data_extxd, behaviour_data_extxdb,
                                                                                ext_input_extxi, example_idxs=example_idxs)

            feed_dict = self.build_feed_dict(name, data_bxtxd, behaviour_data_bxtxdb, ext_input_bxtxi,
                                             keep_prob=keep_prob)
    ...
    def summarize_all(self, datasets, summary_values):
        ...
        tr_behaviour_recon_cost = summary_values['tr_behaviour_recon_cost']
        tr_total_cost = summary_values['tr_total_cost']
        behaviour_weight = summary_values['behaviour_weight']
        kl_weight = summary_values['kl_weight']
        l2_weight = summary_values['l2_weight']
        l2_cost = summary_values['l2_cost']
        orthogonal_cost = summary_values['orthogonal_cost']
        orthogonal_weight = summary_values['orthogonal_weight']
        independence_cost = summary_values['independence_cost']
        independence_weight = summary_values['independence_weight']
        ...
        train_summ, train_step = session.run([self.merged_train,
                                             ...
                                             feed_dict={self.l2_cost_ph: l2_cost,
                                                        self.orthogonal_cost_ph: orthogonal_cost,
                                                        self.independence_cost_ph: independence_cost,
                                                        self.kl_cost_ph: tr_kl_cost,
                                                        self.recon_cost_ph: tr_recon_cost,
                                                        self.behaviour_recon_cost_ph: tr_behaviour_recon_cost,
        ...
        if has_any_valid_set:
            ...
            ev_behaviour_recon_cost = summary_values['ev_behaviour_recon_cost']
            ...
            eval_summ = session.run(self.merged_valid,
                                    feed_dict={self.kl_cost_ph: ev_kl_cost,
                                               ...
                                               self.behaviour_recon_cost_ph: ev_behaviour_recon_cost,
                                               self.total_cost_ph: ev_total_cost})
            self.writer.add_summary(eval_summ, train_step)
            print("Epoch:%d, step:%d (TRAIN, VALID): total: %.2f, %.2f recon: %.2f, %.2f,\
      behaviour recon: %.2f, %.2f,  kl: %.2f, %.2f, l2: %.5f,\
      kl weight: %.2f, l2 weight: %.2f, orthogonal: %.5f, orthogonal weight: %.2f, independence: %.5f, independence weight: %.2f" %
                  (i, train_step, tr_total_cost, ev_total_cost,
                   tr_recon_cost, ev_recon_cost, tr_behaviour_recon_cost,
                   ev_behaviour_recon_cost, tr_kl_cost, ev_kl_cost, l2_cost,
                   kl_weight, l2_weight, orthogonal_cost, orthogonal_weight, independence_cost, independence_weight))

            csv_outstr = "epoch,%d, step,%d, total,%.2f,%.2f, \
      recon,%.2f,%.2f, behaviour recon,%.2f,%.2f, kl,%.2f,%.2f, l2,%.5f, \
      klweight,%.2f, l2weight,%.2f, orthogonal,%.5f, orthogonalweight,%.2f, independence,%.5f, independenceweight,%.2f\n" % \
                (i, train_step, tr_total_cost, ev_total_cost,
                 tr_recon_cost, ev_recon_cost, tr_behaviour_recon_cost, ev_behaviour_recon_cost,
                 tr_kl_cost, ev_kl_cost, l2_cost, kl_weight, l2_weight, orthogonal_cost, orthogonal_weight, independence_cost, independence_weight)

        else:
            print("Epoch:%d, step:%d TRAIN: total: %.2f     recon: %.2f, \
      behaviour recon: %.2f, kl: %.2f, l2: %.5f,    kl weight: %.2f, l2 weight: %.2f, orthogonal: %.5f, orthogonal weight: %.2f, independence: %.5f, independence weight: %.2f" %
                  (i, train_step, tr_total_cost, tr_recon_cost, tr_behaviour_recon_cost, tr_kl_cost,
                   l2_cost, kl_weight, l2_weight, orthogonal_cost, orthogonal_weight, independence_cost, independence_weight))
            csv_outstr = "epoch,%d, step,%d, total,%.2f, recon,%.2f, \
      behaviour recon: %.2f, kl,%.2f, l2,%.5f, klweight,%.2f, l2weight,%.2f, orthogonal,%.5f, orthogonalweight,%.2f, independence,%.5f, independenceweight,%.2f\n" % \
                (i, train_step, tr_total_cost, tr_recon_cost, tr_behaviour_recon_cost,
                 tr_kl_cost, l2_cost, kl_weight, l2_weight, orthogonal_cost, orthogonal_weight, independence_cost, independence_weight)

        if self.hps.csv_log:
            csv_file = os.path.join(
                self.hps.tndm_save_dir, self.hps.csv_log+'.csv')
    ...
    def plot_single_example(self, datasets):
        ...
        train_data_bxtxd, train_behaviour_data_bxtxdb, train_ext_input_bxtxi = \
            self.get_batch(data_dict['train_data'], data_dict['train_behaviours'], data_dict['train_ext_input'],
                           example_idxs=example_idxs)

        truth_train_data_bxtxd = None
        truth_train_behaviour_data_bxtxdb = None
        if (
            'train_truth' in data_dict and data_dict['train_truth'] is not None and
            'train_truth_behaviours' in data_dict and data_dict['train_truth_behaviours'] is not None
        ):
            truth_train_data_bxtxd, truth_train_behaviour_data_bxtxdb, _ = self.get_batch(data_dict['train_truth'],
                                                                                          data_dict['train_truth_behaviours'],
                                                                                          example_idxs=example_idxs)
        ...
        train_model_values = self.eval_model_runs_batch(data_name,
                                                        train_data_bxtxd,
                                                        train_behaviour_data_bxtxdb,
                                                        train_ext_input_bxtxi,
                                                        do_average_batch=False)

        train_step = train_model_values['train_steps']
        feed_dict = self.build_feed_dict(data_name, train_data_bxtxd, train_behaviour_data_bxtxdb,
        ...
        valid_data_bxtxd = valid_behaviour_data_bxtxdb = valid_model_values = valid_ext_input_bxtxi = None
        truth_valid_data_bxtxd = None
        truth_valid_behaviour_data_bxtxdb = None
        if has_valid_set:
            E, _, _ = data_dict['valid_data'].shape
            eidx = np.random.choice(E)
            example_idxs = eidx * np.ones(hps.batch_size, dtype=np.int32)
            valid_data_bxtxd, valid_behaviour_data_bxtxdb, valid_ext_input_bxtxi = \
                self.get_batch(data_dict['valid_data'],
                               data_dict['valid_behaviours'],
                               data_dict['valid_ext_input'],
                               example_idxs=example_idxs)

            if (
                'valid_truth' in data_dict and data_dict['valid_truth'] is not None and
                'valid_truth_behaviours' in data_dict and data_dict[
                    'valid_truth_behaviours'] is not None
            ):
                truth_valid_data_bxtxd, truth_valid_behaviour_data_bxtxdb, _ = self.get_batch(data_dict['valid_truth'],
                                                                                              data_dict['valid_truth_behaviours'],
                                                                                              example_idxs=example_idxs)
            else:
                truth_valid_data_bxtxd = None
                truth_valid_behaviour_data_bxtxdb = None
            ...
            valid_model_values = self.eval_model_runs_batch(data_name,
                                                            valid_data_bxtxd,
                                                            valid_behaviour_data_bxtxdb,
                                                            valid_ext_input_bxtxi,
                                                            do_average_batch=False)

        example_image = plot_tndm(train_bxtxd=train_data_bxtxd,
                                  train_behaviours_bxtxdb=train_behaviour_data_bxtxdb,
                                  train_model_vals=train_model_values,
                                  train_ext_input_bxtxi=train_ext_input_bxtxi,
                                  train_truth_bxtxd=truth_train_data_bxtxd,
                                  train_truth_behaviours_bxtxdb=truth_train_behaviour_data_bxtxdb,
                                  valid_bxtxd=valid_data_bxtxd,
                                  valid_behaviours_bxtxdb=valid_behaviour_data_bxtxdb,
                                  valid_model_vals=valid_model_values,
                                  valid_ext_input_bxtxi=valid_ext_input_bxtxi,
                                  valid_truth_bxtxd=truth_valid_data_bxtxd,
                                  valid_truth_behaviours_bxtxdb=truth_valid_behaviour_data_bxtxdb,
                                  bidx=None, cf=cf, output_dist=hps.output_dist,
                                  behaviour_output_dist=hps.behaviour_output_dist)
    ...
    def train_model(self, datasets):
        ...
        train_costs = []
        valid_costs = []
        ev_total_cost = ev_recon_cost = ev_behaviour_recon_cost = ev_kl_cost = 0.0
        lowest_ev_cost = np.Inf
        while True:
            i += 1
            do_save_ckpt = True if i % 10 == 0 else False
            tr_total_cost, tr_recon_cost, tr_behaviour_recon_cost, tr_kl_cost, behaviour_weight, kl_weight, \
            l2_cost, l2_weight, orthogonal_cost, orthogonal_weight, independence_cost, independence_weight = self.train_epoch(datasets, do_save_ckpt=do_save_ckpt)
            ...
            if has_any_valid_set:
                ev_total_cost, ev_recon_cost, ev_behaviour_recon_cost, ev_kl_cost = \
            ...
            values = {'nepochs': i, 'has_any_valid_set': has_any_valid_set,
                      ...
                      'tr_behaviour_recon_cost': tr_behaviour_recon_cost,
                      'ev_behaviour_recon_cost': ev_behaviour_recon_cost,
                      'tr_kl_cost': tr_kl_cost, 'ev_kl_cost': ev_kl_cost,
                      'behaviour_weight': behaviour_weight, 'l2_weight': l2_weight,
                      'kl_weight': kl_weight, 'l2_cost': l2_cost, 
                      'orthogonal_cost': orthogonal_cost, 'orthogonal_weight': orthogonal_weight,
                      'independence_cost': independence_cost, 'independence_weight': independence_weight}
    ...
    def eval_cost_epoch(self, datasets, kind='train', ext_input_extxi=None,
        ...
        ops_to_eval = [self.cost, self.recon_cost,
                       self.behaviour_recon_cost, self.kl_cost]
        ...
        return epoch_total_cost, epoch_recon_cost, epoch_behaviour_recon_cost, epoch_kl_cost

    def eval_model_runs_batch(self, data_name, data_bxtxd, behaviour_data_bxtxdb, ext_input_bxtxi=None,
        ...
        Args:
          ...
          data_bxtxd:  Numpy array training data with shape:
            batch_size x # time steps x # dimensions
          behaviour_data_bxtxdb:  Numpy array behaviour training data with shape:
            batch_size x # time steps x # behaviour dimensions
        ...
        if E < hps.batch_size:
            ...
            behaviour_data_bxtxdb = np.pad(behaviour_data_bxtxdb, ((0, hps.batch_size-E), (0, 0), (0, 0)),
                                           mode='constant', constant_values=0)
        ...
        feed_dict = self.build_feed_dict(data_name, data_bxtxd, behaviour_data_bxtxdb,
        ...
        tf_vals = [self.gen_ics, self.gen_states, self.rel_gen_ics, self.rel_gen_states, self.factors,
                   self.output_dist_params, self.behaviour_output_dist_params]
        ...
        if self.hps.ic_dim > 0:
            tf_vals += [self.prior_zs_g0.mean, self.prior_zs_g0.logvar,
                        ...
                        self.prior_rel_zs_g0.mean, self.prior_rel_zs_g0.logvar,
                        self.posterior_rel_zs_g0.mean, self.posterior_rel_zs_g0.logvar]
        ...
        ff += 1
        rel_gen_ics = [np_vals_flat[f] for f in fidxs[ff]]
        ff += 1
        rel_gen_states = [np_vals_flat[f] for f in fidxs[ff]]
        ff += 1
        factors = [np_vals_flat[f] for f in fidxs[ff]]
        ff += 1
        out_dist_params = [np_vals_flat[f] for f in fidxs[ff]]
        ff += 1
        behaviour_out_dist_params = [np_vals_flat[f] for f in fidxs[ff]]
        ff += 1
        ...
        if self.hps.ic_dim > 0:
            ...
            ff += 1
        if self.hps.rel_ic_dim > 0:
            prior_rel_g0_mean = [np_vals_flat[f] for f in fidxs[ff]]
            ff += 1
            prior_rel_g0_logvar = [np_vals_flat[f] for f in fidxs[ff]]
            ff += 1
            post_rel_g0_mean = [np_vals_flat[f] for f in fidxs[ff]]
            ff += 1
            post_rel_g0_logvar = [np_vals_flat[f] for f in fidxs[ff]]
            ff += 1
        if self.hps.co_dim > 0:
        ...
        rel_gen_states = list_t_bxn_to_tensor_bxtxn(rel_gen_states)
        factors = list_t_bxn_to_tensor_bxtxn(factors)
        out_dist_params = list_t_bxn_to_tensor_bxtxn(out_dist_params)
        behaviour_out_dist_params = list_t_bxn_to_tensor_bxtxn(
            behaviour_out_dist_params)
        if self.hps.ic_dim > 0:
        ...
        if self.hps.rel_ic_dim > 0:
            # select first time point
            prior_rel_g0_mean = prior_rel_g0_mean[0]
            prior_rel_g0_logvar = prior_rel_g0_logvar[0]
            post_rel_g0_mean = post_rel_g0_mean[0]
            post_rel_g0_logvar = post_rel_g0_logvar[0]
        if self.hps.co_dim > 0:
        ...
        if E < hps.batch_size:
            idx = np.arange(E)
            gen_ics = gen_ics[idx, :]
            rel_gen_ics = rel_gen_ics[idx, :]
            gen_states = gen_states[idx, :]
            rel_gen_states = rel_gen_states[idx, :]
            factors = factors[idx, :, :]
            out_dist_params = out_dist_params[idx, :, :]
            behaviour_out_dist_params = behaviour_out_dist_params[idx, :, :]
            if self.hps.ic_dim > 0:
            ...
            if self.hps.rel_ic_dim > 0:
                prior_rel_g0_mean = prior_rel_g0_mean[idx, :]
                prior_rel_g0_logvar = prior_rel_g0_logvar[idx, :]
                post_rel_g0_mean = post_rel_g0_mean[idx, :]
                post_rel_g0_logvar = post_rel_g0_logvar[idx, :]
            if self.hps.co_dim > 0:
        ...
        if do_average_batch:
            ...
            rel_gen_ics = np.mean(rel_gen_ics, axis=0)
            rel_gen_states = np.mean(rel_gen_states, axis=0)
            factors = np.mean(factors, axis=0)
            out_dist_params = np.mean(out_dist_params, axis=0)
            behaviour_out_dist_params = np.mean(
                behaviour_out_dist_params, axis=0)
            if self.hps.ic_dim > 0:
            ...
            if self.hps.rel_ic_dim > 0:
                prior_rel_g0_mean = np.mean(prior_rel_g0_mean, axis=0)
                prior_rel_g0_logvar = np.mean(prior_rel_g0_logvar, axis=0)
                post_rel_g0_mean = np.mean(post_rel_g0_mean, axis=0)
                post_rel_g0_logvar = np.mean(post_rel_g0_logvar, axis=0)
            if self.hps.co_dim > 0:
        ...
        model_vals['rel_gen_ics'] = rel_gen_ics
        model_vals['rel_gen_states'] = rel_gen_states
        model_vals['factors'] = factors
        model_vals['output_dist_params'] = out_dist_params
        model_vals['behaviour_output_dist_params'] = behaviour_out_dist_params
        ...
        if self.hps.rel_ic_dim > 0:
            model_vals['prior_rel_g0_mean'] = prior_rel_g0_mean
            model_vals['prior_rel_g0_logvar'] = prior_rel_g0_logvar
            model_vals['post_rel_g0_mean'] = post_rel_g0_mean
            model_vals['post_rel_g0_logvar'] = post_rel_g0_logvar
        if self.hps.co_dim > 0:
            model_vals['controller_outputs'] = controller_outputs

        return model_vals

    def eval_model_runs_avg_epoch(self, data_name, data_extxd, behaviour_data_extxdb,
        ...
        Args:
          ...
          data_extxd:  Numpy array training data with shape:
            # examples x # time steps x # dimensions
          behaviour_data_extxdb:  Numpy array behaviour training data with shape:
            # examples x # time steps x # behaviour dimensions
        ...
        batch_size = hps.batch_size
        E, T, D = data_extxd.shape
        _, _, DB = behaviour_data_extxdb.shape
        ...
        if hps.rel_ic_dim > 0:
            prior_rel_g0_mean = np.zeros([E_to_process, hps.rel_ic_dim])
            prior_rel_g0_logvar = np.zeros([E_to_process, hps.rel_ic_dim])
            post_rel_g0_mean = np.zeros([E_to_process, hps.rel_ic_dim])
            post_rel_g0_logvar = np.zeros([E_to_process, hps.rel_ic_dim])
        ...
        rel_gen_ics = np.zeros([E_to_process, hps.rel_gen_dim])
        rel_gen_states = np.zeros([E_to_process, T, hps.rel_gen_dim])
        ...
        if hps.behaviour_output_dist == 'gaussian':
            behaviour_out_dist_params = np.zeros([E_to_process, T, DB+DB])
        elif hps.behaviour_output_dist == 'gaussian_non_temporal' or hps.behaviour_output_dist == 'gaussian_non_temporal_causal':
            behaviour_out_dist_params = np.zeros([E_to_process, T, DB])
        else:
            assert False, "NIY"
        ...
        for es_idx in range(E_to_process):
            ...
            data_bxtxd, behaviour_data_bxtxdb, ext_input_bxtxi = self.get_batch(data_extxd,
                                                                                behaviour_data_extxdb,
                                                                                ext_input_extxi,
                                                                                batch_size=batch_size,
                                                                                example_idxs=example_idxs)
            model_values = self.eval_model_runs_batch(data_name, data_bxtxd,
                                                      behaviour_data_bxtxdb,
                                                      ext_input_bxtxi,
                                                      do_eval_cost=True,
            ...
            if self.hps.ic_dim > 0:
                prior_rel_g0_mean[es_idx, :] = model_values['prior_rel_g0_mean']
                prior_rel_g0_logvar[es_idx, :] = model_values['prior_rel_g0_logvar']
                post_rel_g0_mean[es_idx, :] = model_values['post_rel_g0_mean']
                post_rel_g0_logvar[es_idx, :] = model_values['post_rel_g0_logvar']
                

            if self.hps.co_dim > 0:
                controller_outputs[es_idx, :,
                                   :] = model_values['controller_outputs']
                
            gen_ics[es_idx, :] = model_values['gen_ics']
            gen_states[es_idx, :, :] = model_values['gen_states']
            rel_gen_ics[es_idx, :] = model_values['rel_gen_ics']
            rel_gen_states[es_idx, :, :] = model_values['rel_gen_states']
            
            factors[es_idx, :, :] = model_values['factors']
            out_dist_params[es_idx, :, :] = model_values['output_dist_params']
            behaviour_out_dist_params[es_idx, :,
                                      :] = model_values['behaviour_output_dist_params']
        ...
        if self.hps.ic_dim > 0:
            model_runs['prior_rel_g0_mean'] = prior_rel_g0_mean
            model_runs['prior_rel_g0_logvar'] = prior_rel_g0_logvar
            model_runs['post_rel_g0_mean'] = post_rel_g0_mean
            model_runs['post_rel_g0_logvar'] = post_rel_g0_logvar
        ...
        model_runs['rel_gen_ics'] = rel_gen_ics
        model_runs['rel_gen_states'] = rel_gen_states
        model_runs['factors'] = factors
        model_runs['output_dist_params'] = out_dist_params
        model_runs['behaviour_output_dist_params'] = behaviour_out_dist_params
    ...
    def eval_model_runs_push_mean(self, data_name, data_extxd, behaviour_data_extxdb,
                                  ext_input_extxi=None):
        ...
        for batch_idx, es_idx in enumerate(trial_batches(E_to_process,
            ...
            out_dist_params[es_idx, :, :] = model_values['output_dist_params']
            behaviour_out_dist_params[es_idx, :,
                                      :] = model_values['behaviour_out_dist_params']
    ...
    def write_model_runs(self, datasets, output_fname=None, push_mean=False):
        ...
        TNDM generates a number of outputs for each examples, and these are all
        saved.  They are:
        ...
        for data_name, data_dict in datasets.items():
            data_tuple = [('train', data_dict['train_data'], data_dict['train_behaviours'],
                           data_dict['train_ext_input']),
                          ('valid', data_dict['valid_data'], data_dict['valid_behaviours'],
                           data_dict['valid_ext_input']),
                          ('test', data_dict['test_data'], data_dict['test_behaviours'],
                           None)]
            for data_kind, data_extxd, behaviour_data_extxdb, ext_input_extxi in data_tuple:
                if not output_fname:
                    fname = "model_runs_" + data_name + '_' + data_kind + '_' + kind
                else:
                    fname = output_fname + data_name + '_' + data_kind + '_' + kind

                print("Writing data for %s data and kind %s." %
                      (data_name, data_kind))
                if push_mean:
                    model_runs = self.eval_model_runs_push_mean(data_name, data_extxd, behaviour_data_extxdb,
                                                                ext_input_extxi)
                else:
                    model_runs = self.eval_model_runs_avg_epoch(data_name, data_extxd, behaviour_data_extxdb,
                                                                ext_input_extxi)
                full_fname = os.path.join(hps.tndm_save_dir, fname)
    ...
    def write_model_samples(self, dataset_name, output_fname=None):
        ...
        from the model.
        TNDM generates a number of outputs for each sample, and these are all
        saved.  They are:
          The mean and variance of the prior of g0.
          The control inputs (if enabled)
          The initial conditions, g0, for all examples.
        ...
        tf_vals = [self.factors, self.gen_states, self.gen_ics, self.rel_gen_states, self.rel_gen_ics,
                   self.cost, self.output_dist_params, self.behaviour_output_dist_params]
        if hps.ic_dim > 0:
            tf_vals += [self.prior_zs_g0.mean, self.prior_zs_g0.logvar]
        if hps.rel_ic_dim > 0:
            tf_vals += [self.prior_rel_zs_g0.mean, self.prior_rel_zs_g0.logvar]
        if hps.co_dim > 0:
        ...
        ff += 1
        behaviour_output_dist_params = [np_vals_flat[f] for f in fidxs[ff]]
        ff += 1
        if hps.ic_dim > 0:
            prior_g0_mean = [np_vals_flat[f] for f in fidxs[ff]]
            ff += 1
        ...
        behaviour_output_dist_params = list_t_bxn_to_tensor_bxtxn(
            behaviour_output_dist_params)
        if hps.ic_dim > 0:
            prior_g0_mean = prior_g0_mean[0]
            prior_g0_logvar = prior_g0_logvar[0]
        if hps.rel_ic_dim > 0:
        ...
        model_vals['behaviour_output_dist_params'] = behaviour_output_dist_params
        ...
        full_fname = os.path.join(hps.tndm_save_dir, output_fname)
    ...
    def eval_model_parameters(use_nested=True, include_strs=None):
        ...
        Args:
        use_nested (optional): For returning values, use a nested dictionary, based
